...
public class MovieListViewAdapter extends ArrayAdapter<Movie> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        viewHolder.stars.setText("Stars: " + extractStarNames(movie.getStarsString()));
        viewHolder.genres.setText("Genres: " + movie.getGenresString());
        // Return the completed view to render on screen
        return convertView;
    }

    private static String extractStarNames(String input) {
        ...
        for (String pair : pairs) {
            ...
            // Extract the name (assuming it's at index 1)
            String name = parts[0].trim();
            names.append(name);
            names.append(", ");
        }

        names.delete(names.length() - 2, names.length()); // removes last ", "
