...
//                    "(SELECT GROUP_CONCAT(DISTINCT g.name) FROM ( SELECT DISTINCT g.name FROM genres_in_movies gim RIGHT JOIN genres g ON gim.genreId = g.id WHERE gim.movieId = m.id LIMIT 3 ) AS g LIMIT 3)AS genres, " +
//                    "( SELECT GROUP_CONCAT( CONCAT(s.name, ':', s.id)) FROM ( SELECT DISTINCT s.name, s.id FROM stars_in_movies sim RIGHT JOIN stars s ON sim.starId = s.id WHERE sim.movieId = m.id LIMIT 3 ) AS s LIMIT 3) AS stars, " +
//                    "m.rating FROM (SELECT m.id, m.title, m.year, m.director, r.rating FROM ratings r JOIN movies m ON m.id = r.movieId ORDER BY r.rating DESC LIMIT 20) AS m " +
